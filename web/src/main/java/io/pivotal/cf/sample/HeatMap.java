...
**/
	public void addOrderSum(String state, String result){
			HeatMapItem item = new HeatMapItem();
			item.setState(state);
	...
	public void assignColors(){
		...
		//redColors = redInterpolate(delta+1);
		//blueColors = blueInterpolate(delta+1);
		...
		while (it.hasNext()){
			...
			if(result.equals("red")){
				item.setHeatMapColor("#A11437");
				//item.setHeatMapColor("#"+convertRGBToHex(redColors[itemColorGradient].getRed()+","+redColors[itemColorGradient].getGreen()+","+redColors[itemColorGradient].getBlue()));
			}else if(result.equals("blue")){
				item.setHeatMapColor("#0078FF");
				//item.setHeatMapColor("#"+convertRGBToHex(blueColors[itemColorGradient].getRed()+","+blueColors[itemColorGradient].getGreen()+","+blueColors[itemColorGradient].getBlue()));
			}
		//	i++;
		}
	...
	private Color[] redInterpolate(int steps){

		Color[] colors = new Color[steps];

        final Color WHITE = new Color(255,255,255);
        final Color PIVOTAL = new Color(161,20,55);
...
private Color[] blueInterpolate(int steps){

		Color[] colors = new Color[steps];

        final Color WHITE = new Color(255,255,255);
        final Color PIVOTAL = new Color(0,120,255);
